...
async def main():
    ...
    async with async_playwright() as p:
        ...
        await page.screenshot(path=os.path.join(output_dir, "v_final_pralnia.png"), full_page=False)

        # 2. Sprzatanie (Night)
        await page.goto(f"http://localhost:{PORT}/sprzatanie.html", wait_until="networkidle")
        await page.screenshot(path=os.path.join(output_dir, "v_final_sprzatanie.png"), full_page=False)

        # 3. Magazyny (Night)
        await page.goto(f"http://localhost:{PORT}/magazyny.html", wait_until="networkidle")
        await page.screenshot(path=os.path.join(output_dir, "v_final_magazyny.png"), full_page=False)

        # 4. Helipad (Night)
        await page.goto(f"http://localhost:{PORT}/helipad.html", wait_until="networkidle")
        await page.screenshot(path=os.path.join(output_dir, "v_final_helipad.png"), full_page=False)

        # 5. Apartamenty (Night)
        await page.goto(f"http://localhost:{PORT}/apartamenty.html", wait_until="networkidle")
        await page.screenshot(path=os.path.join(output_dir, "v_final_apartamenty.png"), full_page=False)

        # 6. Index (Night)
        await page.goto(f"http://localhost:{PORT}/index.html", wait_until="networkidle")
        await page.screenshot(path=os.path.join(output_dir, "v_final_index.png"), full_page=False)

        await browser.close()
        print("All final pages verified!")
